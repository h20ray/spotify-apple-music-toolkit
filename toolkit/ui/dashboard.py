...
def display_dashboard():
    ...
    header_text = f"""[bold cyan]SPOTIFY - APPLE MUSIC TOOLKIT[/bold cyan]
Unified Master Dashboard for Playlists, Audio Tagging & Synced Lyrics

- {sp_status}
- [bold white]Text Playlist Files:[/bold white] [bold magenta]{len(txt_files)} file(s)[/bold magenta] in [dim]playlist_sources/source_text_files/[/dim]
...
def scan_workspace_overview():
    ...
    txt_files = get_all_txt_files()
    t1 = Table(title="Playlist Text Files (playlist_sources/source_text_files/)", border_style="magenta", header_style="bold cyan")
